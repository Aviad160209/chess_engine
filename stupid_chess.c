...
int main()
{
    ...
    while (amountOfMoves == 0)
    {
                ...
                for (col = 0; col < 8; col++)
                {
                    if (chessBord[row][col] == 'r') 
                    {
                        ...
                        if (col == targetCol) 
                        {
                            ...
                            if (clear) 
                            {
                                if (chessBord[targetRow][col] != '.' && chessBord[targetRow][col] >= 'b' && chessBord[targetRow][col] <= 'p') 
                                {
                                    break; // ניסיון לאכול את עצמך – לא נחשב מהלך חוקי
                                }
                        ...
                        else if (row == targetRow) 
                        {
                            ...
                            if (clear) 
                            {
                                if (chessBord[row][targetCol] != '.' && chessBord[row][targetCol] >= 'b' && chessBord[row][targetCol] <= 'p') 
                                {
                                    break; // ניסיון לאכול את עצמך – לא נחשב מהלך חוקי
                                }
        ...
        // הדפסת מצב הלוח לאחר ניסיון תנועה
        for (row = 0; row < 8; row++)
        {
            for (col = 0; col < 8; col++)
            {
